...
/**
   * @title HODL
  */
contract HODL is StandardToken {

    // FIELDS

    string public name = "HODL Fund";
    string public symbol = "HODL";
    uint256 public decimals = 18;
    string public version = "1.0";

    // Ninety million maximum token supply with suport for 18 decimals
    uint256 public tokenCap = 90000000 * 10**18;
    ...
    // tradeable: signal that HODL Fund is ready to operate and underlying assets have been acquired
    ...
    modifier onlyWhitelist {
        require(whitelist[msg.sender] || !requireWhitelist);
        _;
    }
    ...
    function HODL(address controlWalletInput, uint256 priceNumeratorInput, uint256 startBlockInput, uint256 endBlockInput) {
    ...
    function allocateTokens(address participant, uint256 amountTokens) private {
        require(vestingSet);
        // 16.5% of total allocated to the Company and team for product development, marketing, fund reserves
        uint256 developmentAllocation = safeMul(amountTokens, 1.65 * 10 ** 16) / 100000000000000000;
    ...
    function requireWhitelist() external onlyFundWallet {
        requireWhitelist = true;
    }
    function unrequireWhitelist() external onlyFundWallet {
...
contract HODLVesting is SafeMath {
    ...
    bool private fourthDone = false;
    uint256 public fifthRelease;
    ...
    // in total 16.5% of HODL tokens will be sent to this contract
    // COMPANY ALLOCATION: 9.5%
    //   2.5% - Fund reserves
    //   2.5% - Development of HNS
    //   0.75% - Legal 
    //   0.5% - Referral program
    //   1.5% - Marketing, investor relations and advisors 
    // TEAM ALLOCATION (vest in 5 stages over 2 years)
    //   2.5% to the CEO
    //   2.25% to the CTO
    //   2.25% to the CMO
    
    // Initial claim is Company expenses 
    // (9.5/16.5 = 57.5757576% of the tokens)
    // each other release (for team) is 8.48484848% of tokens sent here
    ...
    function init_claim(uint256 balance) private atStage(Stages.initClaim) {
        firstRelease = now + 20 weeks; // assign 5 claiming times
        secondRelease = firstRelease + 20 weeks;
        thirdRelease = secondRelease + 20 weeks;
        fourthRelease = thirdRelease + 20 weeks;
        fifthRelease = fourthRelease + 20 weeks;
        uint256 amountToTransfer = safeMul(balance, 57575757600) / 100000000000;
        ERC20Token.transfer(beneficiary, amountToTransfer); // now 42.4242424% tokens left
        nextStage();
    }
    function first_release(uint256 balance) private atStage(Stages.firstRelease) {
        ...
        ERC20Token.transfer(beneficiary, amountToTransfer); // send 20 % of team releases
        nextStage();
    }
    ...
    function fifth_release(uint256 balance) private atStage(Stages.fifthRelease) {
        require(now > fifthRelease);
        ERC20Token.transfer(beneficiary, balance); // send remaining 20 % of team releases
    }
